...
const makeUserInsertSQL = async (seq) => {
  const userPrefix = `INSERT INTO mydb.users (id, password, name, nickname, phone_number, user_status, region_id, reward, email, created_time, updated_time, withdrawal_date) VALUES `;
  ...
  try {
    ...
    const userSql = `${userPrefix} ( '${userId}', '${password}', '${name}', '${nickname}', '${phoneNumber}', 'JOINED', ${regionId}, 10, '${email}', '${now}', '${now}', null)`;
